...
public class SensorSimulationService {
    ...
    public List<SensorReading> getReadingHistory(int limit, String zoneName) {
        ...
        if (StringUtils.hasText(zoneName)) {
            return sensorReadingRepository
                    .findRecentByZoneWithDetails(zoneName, cutoff, PageRequest.of(0, sanitizedLimit))
                    .stream()
        ...
        return sensorReadingRepository
                .findRecentWithDetails(cutoff, PageRequest.of(0, sanitizedLimit))
                .stream()
                .map(this::toModelReading)
                .toList();
    }
    ...
    public ForestMapSnapshot getForestMapSnapshot(String username, String role) {
        ...
        Map<String, SensorEntity> sensorMap = new HashMap<>();
        for (SensorEntity s : allSensors) {
            sensorMap.put(s.getSensorId(), s);
        }

        List<ForestOutpostEntity> allOutpostEntities = outpostRepository.findAll();
        Map<String, ForestOutpostEntity> outpostMapByZone = new HashMap<>();
        for (ForestOutpostEntity o : allOutpostEntities) {
            outpostMapByZone.put(o.getZoneName().toLowerCase(), o);
        }
        ...
        for (ZoneStatus zone : zones) {
            ...
            ForestOutpostEntity outpostEntity = outpostMapByZone.get(zone.getZoneName().toLowerCase());
